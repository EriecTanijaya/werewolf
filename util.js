    ...
    buttons: [
      {
        action: "uri",
        label: "🚪 Open forum",
        data: "https://line.me/ti/g2/3NEqw4h7jNdOBCur8AQWyw?utm_source=invitation&utm_medium=link_copy&utm_campaign=default",
      },
    ],
  };
  return flex_text;
...
const leaveGroup = async (event, groupId, text) => {
  try {
    await client.replyMessage(event.replyToken, { type: "text", text });
    if (event.source.type === "group") {
      await client.leaveGroup(groupId);
    } else {
      await client.leaveRoom(groupId);
    }
  } catch (err) {
    console.error(err);
  }
